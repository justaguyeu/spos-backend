...
#     "127.0.0.1,localhost,spos-backend-sdpz.onrender.com,spostz.netlify.app",
# ).split(",")
...
#     "http://localhost:3000,https://spostz.netlify.app, https://spos-backend-sdpz.onrender.com",
# ).split(",")
...
DATABASES = {
    "default": dj_database_url.parse(
        os.getenv(
            "DATABASE_URL",
            "postgresql://sposdatabase_user:HvifBR8DPHkvGzrVCP5zIbQn6rGOVMJC@dpg-d7mdogreo5us73eoj2ug-a.oregon-postgres.render.com/sposdatabase",
        ),
        conn_max_age=60,
...
# CACHE
# ---------------------------------------------------------------------------
# With 1000+ users, local memory cache is NOT shared between workers.
# Every Render worker has its own counter → throttle limits multiply by
# number of workers → effectively useless.
# Use Redis so ALL workers share ONE counter.
#
# Set REDIS_URL in your Render environment variables.
# Free options: Upstash Redis (https://upstash.com) — 10k req/day free.
# ---------------------------------------------------------------------------
_redis_url = os.getenv("REDIS_URL", "")
...
if _redis_url:
    ...
    # Local dev fallback — single process so counters are accurate enough
    CACHES = {
        "default": {
            "BACKEND":  "django.core.cache.backends.locmem.LocMemCache",
            "LOCATION": "spos-cache",
...
REST_FRAMEWORK = {
    ...
    # ---------------------------------------------------------------------------
    # THROTTLE RATES — tuned for 1000+ users/hour
    #
    # Maths:
    #   1000 users/hour, each user loads ~5 pages, each page fires ~10 requests
    #   = 50 000 requests/hour total across all users
    #   = ~834 requests/minute total
    #
    # Per-company burst:   300/minute  (a company's whole team loading dashboards)
    # Per-company hour:    5000/hour   (a busy company doing data entry all day)
    # Anon (login/register): 30/minute per IP (brute-force protection)
    # Token refresh:       10/minute per IP (stops the 429 loop you were seeing)
    # ---------------------------------------------------------------------------
    "DEFAULT_THROTTLE_CLASSES": [
        "myapp.throttles.CompanyBurstThrottle",
        "myapp.throttles.CompanySustainedThrottle",
    ...
    "DEFAULT_THROTTLE_RATES": {
        "company_burst":     "300/minute",   # raised from 60
        "company_sustained": "5000/hour",    # raised from 1000
        "strict_anon":       "30/minute",    # raised from 10
        "token_refresh":     "10/minute",    # separate scope for refresh endpoint
    },
